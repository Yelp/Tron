...
class ValidateAction(Validator):
    ...
    validators = {
        ...
        "on_upstream_rerun": config_utils.build_real_enum_validator(
            schema.ActionOnRerun
        ),
...
class ValidateStatePersistence(Validator):
    ...
    validators = {
        "name": valid_string,
        "store_type": config_utils.build_real_enum_validator(
...
DEFAULT_STATE_PERSISTENCE = ConfigState(
    name="tron_state", store_type="shelve", buffer_size=1,
)
...
class ValidateConfig(Validator):
    ...
    def post_validation(self, config, _):
        ...
        config_context = ConfigContext(
            "config", node_names, config.get("command_context"), MASTER_NAMESPACE,
        )
