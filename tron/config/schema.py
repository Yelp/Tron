...
NamedTronConfig = config_object_factory(
    name="NamedTronConfig", optional=["jobs",],  # dict of ConfigJob
)
...
ConfigNode = config_object_factory(
    name="ConfigNode", required=["hostname"], optional=["name", "username", "port"],
)
...
ConfigState = config_object_factory(
    ...
    optional=["buffer_size", "dynamodb_region", "table_name",],
)
...
ConfigParameter = config_object_factory(
    name="ConfigParameter", required=["key", "value",], optional=[],
)
...
ActionRunnerTypes = Enum(
    "ActionRunnerTypes", dict(none="none", subprocess="subprocess")
)
