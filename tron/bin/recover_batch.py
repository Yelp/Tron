...
def get_exit_code(filepath):
    ...
    if return_code is not None:
        if return_code < 0:
            ...
            error_message = (
                f"Action run killed by signal {signal.Signals(exit_code).name}"
            )
        else:
            exit_code = return_code
    elif pid is None:
        log.warning(
            f"Status file {filepath} didn't have a PID. Will watch the file for updates."
        )
    elif not psutil.pid_exists(pid):
        exit_code = 1
        error_message = (
            f"Action runner pid {pid} no longer running. Assuming an exit of 1."
        )
