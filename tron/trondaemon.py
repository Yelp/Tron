...
class TronDaemon(object):
    ...
    def __init__(self, options):
        ...
        self.mcp = None
        # This must happen before the context is built
    ...
    def _build_context(self, options):
        return NoDaemonContext(
            ...
            # We have the context set all the signal handlers here to defaults
            # just in case someone else sets them differently. We do this so
            # that we can handle them synchronously in _run_reactor
            signal_map={
    ...
    def run(self):
        # Distinct from the context's signal map. That is used to set signal
        # handlers in Python. We use the ones below to synchronously
        signal_map = {
        ...
        while True:
            ...
            if signum in signal_map:
                log.info(f"Got signal {str(signum)}")
