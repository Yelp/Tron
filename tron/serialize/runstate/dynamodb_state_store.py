...
# Max DynamoDB object size is 400KB. Since we save two copies of the object (pickled and compressed JSON),
# we need to consider this max size applies to the entire item. For reasons that I'm not too interested
# in investigating, when switching to compressed JSON we found that we were still exceeding the max item
# size when using OBJECT_SIZE 200_000. To be safe, we're going to limit our max partition size well below
# that for now. This is more than enough room for even our largest JobRuns.
...
class DynamoDBStateStore:
    ...
    def _serialize_item(self, key: Literal[runstate.JOB_STATE, runstate.JOB_RUN_STATE], state: Dict[str, Any]) -> Optional[bytes]:  # type: ignore
        try:
    ...
    def __setitem__(self, key: str, value: Tuple[bytes, Optional[bytes]]) -> None:
        """
