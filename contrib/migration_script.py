...
def parse_args():
    ...
    parser.add_argument("--old-ns", required=True, help="Old namespace")
    parser.add_argument("--new-ns", required=True, help="New namespace")
    parser.add_argument("source", help="source file to get list of jobs")
    parser.add_argument("--job", help="Specify a single job to migrate")
    ...
    for job_status in jobs_status:
        if job_status["name"] == job_name:
            ...
            if status == "running":
                print(
                    bcolors.FAIL
                    + "job {} is still running, can not migrate".format(job_name)
                    + bcolors.ENDC
                )
                return False
                ...
                print(
                    bcolors.WARNING
                    + "job {} is disabled, need to cancel it manually later".format(
                        job_name
                    )
                    + bcolors.ENDC
                )
                ...
                print(
                    bcolors.OKGREEN
                    + "job {} is not running, can migrate".format(job_name)
                    + bcolors.ENDC
                )
                return True
...
def command_jobs(command, jobs, args, ns=None):
    ...
    for job in jobs:
        ...
        if response.error:
            print(
                bcolors.FAIL
                + "Failed to {} {}".format(command, job_name)
                + bcolors.ENDC
            )
            command_flag = False
        else:
            print(
                bcolors.OKGREEN
                + "Succeed to {} {}".format(command, job_name)
                + bcolors.ENDC
            )
    return command_flag
...
def ssh_command(hostname, command):
    print(
        bcolors.BOLD
        + "Executing the command: ssh -A {} {}".format(hostname, command)
        + bcolors.ENDC
    )
    ...
    if exitcode != 0:
        print(
            bcolors.FAIL
            + "Execute command {} failed: {}".format(command, error)
            + bcolors.ENDC
        )
        exit(exitcode)
    return result
    ...
    if filename.endswith(".yaml"):
        ...
        with open(filename, "r") as f:
            ...
            if args.job is not None:  # only want to migrate specific job
                ...
                is_migration_safe = is_migration_safe & check_job_if_running(
                    jobs_status, job_name_with_ns
                )
        ...
        if is_migration_safe is True:
            ...
            print(
                bcolors.WARNING
                + "Some jobs are still running, abort this migration,"
                + bcolors.ENDC
            )
            return
