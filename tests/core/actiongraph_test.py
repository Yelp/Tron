...
class ActionGraphTestCase(TestCase):
    ...
    def setup_graph(self):
        ...
        self.action_map = ActionMap.from_config(
            [
                {'name': 'base_one', 'command': ''},
                {'name': 'base_two', 'command': ''},
                {'name': 'dep_one', 'command': '', 'requires': ['base_one']},
                {
                    'name': 'dep_one_one', 'command': '',
                    'requires': ['dep_one'],
                },
                {
                    'name': 'dep_multi', 'command': '',
    ...
    def test_from_config(self):
        ...
        assert_equal(
            self.action_graph['dep_one'].dependent_actions, {'dep_one_one'},
        )
        assert_equal(
            self.action_graph['dep_one_one'].required_actions, {'dep_one'},
        )

    def test__eq__(self):
        ag = actiongraph.ActionGraph.from_config(
            self.action_map, self.config_context,
        )
