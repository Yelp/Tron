...
class TestActionGraph(TestCase):
    ...
    def setup_graph(self):
        ...
        self.action_graph = actiongraph.ActionGraph(
            self.action_map, self.required_actions, self.required_triggers
        )
    ...
    def test_get_dependencies(self):
        ...
        assert (
            self.action_graph.get_dependencies("base_one", include_triggers=True)[
                0
            ].name
            == "MASTER.otherjob.first"
        )
        assert sorted(
            [d.name for d in self.action_graph.get_dependencies("dep_multi")]
        ) == sorted(["dep_one_one", "base_two",])
