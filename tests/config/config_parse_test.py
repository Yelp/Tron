...
def make_mock_schedule():
    return ConfigDailyScheduler(
        days=set(), hour=0, minute=0, second=0, original="00:00:00 ", jitter=None,
    )
...
def make_nodes():
    return {
        "node0": schema.ConfigNode(
            name="node0", username="foo", hostname="node0", port=22,
        ),
        "node1": schema.ConfigNode(
            name="node1", username="foo", hostname="node1", port=22,
        ),
    }
...
def make_master_jobs():
    return {
        ...
        "MASTER.test_job1": make_job(
            ...
            actions={
                "action": make_action(
                    requires=("action1",), expected_runtime=datetime.timedelta(0, 7200)
                ),
                "action1": make_action(
                    name="action1", expected_runtime=datetime.timedelta(0, 7200)
                ),
            },
        ...
        "MASTER.test_job2": make_job(
            ...
            node="node1",
            actions={
                "action2_0": make_action(name="action2_0", command="test_command2.0",)
            },
            time_zone=pytz.timezone("Pacific/Auckland"),
            expected_runtime=datetime.timedelta(1),
            cleanup_action=None,
        ...
        "MASTER.test_job_actions_dict": make_job(
            ...
            actions={
                ...
                "action2": make_action(
                    name="action2", requires=("action", "action1"), node="node0",
                ),
            },
            cleanup_action=None,
            expected_runtime=datetime.timedelta(1),
...
def make_tron_config(
    ...
    return schema.TronConfig(
        ...
        or dict(batch_dir="/tron/batch/test/foo", python="/usr/bin/python"),
        ...
        jobs=[
            ...
            dict(
                ...
                actions=dict(
                    ...
                    action2=dict(
                        node="node0", command="command", requires=["action", "action1"]
                    ),
                ),
            ),
            dict(
...
class TestNamedConfig(TestCase):
    ...
    def test_attributes_with_master_context(self):
        ...
        master_config = dict(
            nodes=[dict(name="node0", hostname="node0",)],
            node_pools=[dict(name="nodepool0", nodes=["node0"],)],
        )
        test_config = validate_fragment(
            "test_namespace",
            dict(
...
class TestJobConfig(TestCase):
    def test_no_actions(self):
        test_config = dict(
            jobs=[dict(name="test_job0", node="node0", schedule="daily 00:30:00 ")],
            **BASE_CONFIG,
        )
    ...
    def test_validate_job_no_actions(self):
        job_config = dict(
            name="job_name", node="localhost", schedule="daily 00:30:00", actions=[],
        )
...
class TestNodeConfig(TestCase):
    ...
    def test_invalid_node_name(self):
        ...
        expected_msg = (
            "Unknown node name unknown_node at config.jobs.Job.test_job0.node"
        )
        exception = assert_raises(ConfigError, valid_config, test_config,)
        assert_equal(expected_msg, str(exception))
...
class TestValidMesosAction(TestCase):
    ...
    def test_cleanup_missing_docker_image(self):
        config = dict(
            command="echo hello", executor="mesos", cpus=0.2, mem=150, disk=450,
        )
        with pytest.raises(ConfigError):
            config_parse.valid_action(config, NullConfigContext)
...
class TestValidOutputStreamDir(TestCase):
    ...
    def test_missing_dir(self):
        exception = assert_raises(
            ConfigError, valid_output_stream_dir, "bogus-dir", NullConfigContext,
        )
...
class TestBuildFormatStringValidator(TestCase):
    ...
    def test_validator_unknown_variable_error(self):
        ...
        exception = assert_raises(
            ConfigError, self.validator, template, NullConfigContext,
        )
    ...
    def test_validator_valid_string_without_no_percent_escape(self):
        ...
        context = config_utils.ConfigContext(
            path=None, nodes=None, command_context={"mars": "ok"}, namespace=None,
        )
...
class TestValidateIdentityFile(TestCase):
    ...
    def test_valid_identity_files_missing_public_key(self):
        ...
        exception = assert_raises(
            ConfigError, config_parse.valid_identity_file, filename, self.context,
        )
