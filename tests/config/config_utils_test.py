...
class TestValidTimeDelta(TestCase):
    ...
    def test_valid_time_delta_invalid(self):
        exception = assert_raises(
            ConfigError, config_utils.valid_time_delta, "no time", self.context,
        )
...
StubConfigObject = schema.config_object_factory(
    "StubConfigObject", ["req1", "req2"], ["opt1", "opt2"],
)
...
class TestValidator(TestCase):
    ...
    def test_validate_with_none(self):
        ...
        exception = assert_raises(
            ConfigError, self.validator.validate, None, config_utils.NullConfigContext,
        )
