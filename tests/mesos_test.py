...
class TestMesosClusterRepository(TestCase):
    ...
    def mock_cluster(self):
        ...
        with mock.patch(
            "tron.mesos.MesosCluster", side_effect=init_cluster, autospec=True,
        ) as self.cluster_cls:
            yield
    ...
    def test_shutdown(self):
        clusters = [
            MesosClusterRepository.get_cluster(address) for address in ["a", "b", "c"]
        ]
    ...
    def test_configure(self):
        clusters = [
            MesosClusterRepository.get_cluster(address) for address in ["d", "e"]
        ]
...
class TestMesosTask(TestCase):
    ...
    def test_handle_finished(self):
        ...
        event = mock_task_event(
            task_id=self.task_id, platform_type="finished", terminal=True, success=True,
        )
        self.task.handle_event(event)
    ...
    def test_handle_failed(self):
        self.task.started()
        event = mock_task_event(
            task_id=self.task_id, platform_type="failed", terminal=True, success=False,
        )
        self.task.handle_event(event)
    ...
    def test_handle_killed(self):
        self.task.started()
        event = mock_task_event(
            task_id=self.task_id, platform_type="killed", terminal=True, success=False,
        )
        self.task.handle_event(event)
        assert self.task.is_failed
        assert self.task.is_done
    ...
    def test_handle_lost(self):
        self.task.started()
        event = mock_task_event(
            task_id=self.task_id, platform_type="lost", terminal=True, success=False,
        )
        self.task.handle_event(event)
    ...
    def test_handle_error(self):
        self.task.started()
        event = mock_task_event(
            task_id=self.task_id, platform_type="error", terminal=True, success=False,
        )
        self.task.handle_event(event)
        assert self.task.is_failed
        assert self.task.is_done
    ...
    def test_handle_success_sequence(self):
        self.task.handle_event(
            mock_task_event(task_id=self.task_id, platform_type="staging",)
        )
        self.task.handle_event(
            mock_task_event(task_id=self.task_id, platform_type="starting",)
        )
        self.task.handle_event(
            mock_task_event(task_id=self.task_id, platform_type="running",)
        )
        self.task.handle_event(
            mock_task_event(
                task_id=self.task_id,
    ...
    def test_log_event_error(self):
        with mock.patch.object(
            self.task, "log_event_info",
        ) as mock_log_event, mock.patch.object(self.task.log, "warning",) as mock_log:
...
class TestMesosCluster(TestCase):
    ...
    def test_submit(self):
        ...
        mock_task = mock.MagicMock(
            get_config=mock.Mock(return_value={"environment": {}})
        )
