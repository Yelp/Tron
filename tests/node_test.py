...
class TestNode(TestCase):
    ...
    def test_from_config(self):
        ...
        node_config = mock.Mock(
            hostname="localhost", username="theuser", name="thename",
        )
    ...
    def test_stop_not_tracked(self):
        action_command = mock.create_autospec(
            actioncommand.ActionCommand, id=mock.Mock(),
        )
...
class TestNodePool(TestCase):
    ...
    def test_next_round_robin(self):
        node_order = [
            self.node_pool.next_round_robin() for _ in range(len(self.nodes) * 2)
        ]
