...
test_in_docker_%: docker_%
	$(DOCKER_RUN) tron-builder-$* python3.12 -m tox -vv -e py312
...
dev:
	SSH_AUTH_SOCK=$(SSH_AUTH_SOCK) .tox/py312/bin/trond --debug --working-dir=dev -l logging.conf --host=0.0.0.0

example_cluster:
	tox -e example-cluster

yelpy:
	.tox/py312/bin/pip install -r yelp_package/extra_requirements_yelp.txt
